...
mostrar_detalle_completo = lambda r: (
    ...
    # Info básica
    tk.Label(frame_contenido_detalle, text=f"Categoría: {r['categoria']} | Tiempo: {r['tiempo']} min | Porciones base: {r.get('porciones', 'N/A')}", 
    ...
    tk.Label(frame_porciones, text="Ajustar porciones:", font=("Arial", 11, "bold"), bg=COLOR_FONDO, fg=COLOR_TEXTO).pack(side=tk.LEFT, padx=5),
    (spinbox_porciones := tk.Spinbox(frame_porciones, from_=1, to=20, font=("Arial", 10), width=5)),
    ...
    spinbox_porciones.bind("<KeyRelease>", lambda e: actualizar_ingredientes(spinbox_porciones, lista_ing_detalle, r)),
    ...
    tk.Button(frame_acciones, text="💾 Guardar como TXT", font=("Arial", 10, "bold"), bg=COLOR_BOTON, fg="white",
              command=lambda: guardar_receta_txt(r)).pack(side=tk.LEFT, padx=5),
)

actualizar_ingredientes = lambda sp, lista, r: (
...
PORCIONES: {r.get('porciones', 'N/A')}

DESCRIPCIÓN:
{r.get('descripcion', '')}

INGREDIENTES:
{chr(10).join(f"- {ing}" for ing in r.get('ingredientes', []))}
